...
//		table.getColumnModel().getColumn(2).setCellRenderer(new ReturnTableCellRenderer());
	...
	/*
	 * public class ReturnTableCellRenderer extends JLabel implements
	 * TableCellRenderer { public Component getTableCellRendererComponent(JTable
	 * table, Object value, boolean isSelected, boolean hasFocus, int row, int
	 * column) { if (value==null) return this; setText(value.toString());
	 * setOpaque(true); setHorizontalAlignment(JLabel.CENTER);
	 * 
	 * if (Integer.parseInt(table.getValueAt(row, 2).toString())>=90) {
	 * setBackground(Color.CYAN); }else if(Integer.parseInt(table.getValueAt(row,
	 * 2).toString())>=80) { setBackground(Color.LIGHT_GRAY); } else {
	 * setBackground(Color.WHITE); } if (isSelected) { setBackground(Color.orange);
	 * } return this; } }
	 */
